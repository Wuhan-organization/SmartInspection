...
public class SwipeBackLayout extends FrameLayout {
	...
	private static final int OVERSCROLL_DISTANCE = 40;// dp
	...
	public SwipeBackLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		mOverscrollDistance = (int) (OVERSCROLL_DISTANCE * density + .5f);
	}
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			if ((mTrackingEdge & EDGE_LEFT) != 0) {
				left = xvel > 0 || xvel == 0
						...
						+ mShadowLeft.getIntrinsicWidth() + mOverscrollDistance
						: 0;
			} else if ((mTrackingEdge & EDGE_RIGHT) != 0) {
				left = xvel < 0 || xvel == 0
						&& mScrollPercent > mScrollThreshold ? -(childWidth
				...
				top = yvel < 0 || yvel == 0
						...
						+ mShadowBottom.getIntrinsicHeight() + mOverscrollDistance)
						: 0;
			}
